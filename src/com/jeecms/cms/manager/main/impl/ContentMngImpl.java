...
public class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	public Pagination getPageByRight(String title, String author,
			...
			Integer inputUserId, Integer inputDepartmentId, boolean topLevel,
			boolean recommend, ContentStatus status, Byte checkStep,
			Integer siteId, Integer channelId, Integer userId, int orderBy,
		...
		boolean superAdmin = user.getSuperAdmin();// 判断是否是超级管理员
		if (allChannel && selfData) {
			// 拥有所有栏目权限，只能管理自己的数据
			p = dao.getPageBySelf(title, author, editor, approver, typeId,
					inputUserId, topLevel, recommend, status, checkStep,
					siteId, channelId, userId, orderBy, pageNo, pageSize);
		} else if (allChannel && !selfData) {
			// 拥有所有栏目权限，能够管理不属于自己的数据
			p = dao.getPage(title, author, editor, approver, typeId, userId,
					inputUserId, inputDepartmentId, topLevel, recommend,
					status, checkStep, siteId, null, channelId, orderBy,
	...
	public Pagination getPageBySite(String title, Integer typeId,
			...
			ContentStatus status, Integer siteId, int orderBy, int pageNo,
			int pageSize) {
		// 目前默认部门id为null，非超级管理员
		// 作者 编辑 审批目前均为null
		return dao.getPage(title, null, null, null, typeId, null, inputUserId,
				null, topLevel, recommend, status, null, siteId, null, null,
				orderBy, pageNo, pageSize, false);
	}

	public Pagination getPageForMember(String title, Integer channelId,
			Integer siteId, Integer modelId, Integer memberId, int pageNo,
			int pageSize) {
		// 目前默认部门id为null，非超级管理员
		// 作者 编辑 审批目前均为null
		return dao.getPage(title, null, null, null, null, memberId, memberId,
				null, false, false, ContentStatus.all, null, siteId, modelId,
				channelId, 0, pageNo, pageSize, false);
	}
	...
	public Pagination getPageBySiteIdsForTag(Integer[] siteIds,
			...
			String title, Map<String, String[]> attr, int orderBy, int pageNo,
			int pageSize) {
	...
	public Pagination getPageByChannelIdsForTag(Integer[] channelIds,
			Integer[] typeIds, Boolean titleImg, Boolean recommend,
			String title, Map<String, String[]> attr, int orderBy, int option,
			int pageNo, int pageSize) {
	...
	public Pagination getPageByTopicIdForTag(Integer topicId,
		...
		return dao.getPageByTopicIdForTag(topicId, siteIds, channelIds,
				typeIds, titleImg, recommend, title, attr, orderBy, pageNo,
				pageSize);
	}
	...
	public List<Content> getListByTopicIdForTag(Integer topicId,
		...
		return dao.getListByTopicIdForTag(topicId, siteIds, channelIds,
				typeIds, titleImg, recommend, title, attr, orderBy, first,
				count);
	}
	...
	public Content save(Content bean, ContentExt ext, ContentTxt txt,
			...
			Integer typeId, Boolean draft, Boolean contribute, CmsUser user,
			boolean forMember) {
		saveContent(bean, ext, txt, channelId, typeId, draft, contribute, user,
				forMember);
		// 保存副栏目
	...
	public Content save(Content bean, ContentExt ext, ContentTxt txt,
			...
			boolean forMember) {
		saveContent(bean, ext, txt, channelId, typeId, draft, false, user,
				forMember);
		// 执行监听器
		afterSave(bean);
		return bean;
	...
	private Content saveContent(Content bean, ContentExt ext, ContentTxt txt,
		...
		check = contentCheckMng.save(check, bean);
		// 添加审核记录
		contentCheckRecordMng.save(ContentCheckRecord.cloneContentCheck(check,
				user));
	...
	public Content check(Integer id, CmsUser user, String editor,
			String approver) {
		...
		check.setCheckStep(userStep);
		content.setStatus(ContentCheck.CHECKING);// 审核时将审核状态统一设置成审核中，包括投稿来的稿件 by
													// 聂箫 20160106
		// 终审
	...
	public Content[] check(Integer[] ids, CmsUser user, String editor,
			String approver) {
		Content[] beans = new Content[ids.length];
		for (int i = 0, len = ids.length; i < len; i++) {
			beans[i] = check(ids[i], user, editor, approver);
		}
		return beans;
	}
		...
		for (int i = 0, len = ids.length; i < len; i++) {
			...
			try {
				...
				if (!content.isChecked()) {
					throw new ContentNotCheckedException("content.notChecked",
	...
	@Autowired
	public void setContentCheckRecordMng(
			ContentCheckRecordMng contentCheckRecordMng) {
		this.contentCheckRecordMng = contentCheckRecordMng;
	}

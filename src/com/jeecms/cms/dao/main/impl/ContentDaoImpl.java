...
public class ContentDaoImpl extends HibernateBaseDao<Content, Integer>
	...
	public Pagination getPage(String title, String author, String editor,
			String approver, Integer typeId, Integer currUserId,
	 ...
	 * 
	 * @param f
	 * @param currDepartmentId
	 *            当前用户部门id
	 * @param inputDepartmentId
	 *            内容文章录入用户所在部门id
	 * @param superAdmin
	 *            当前用户是否是超级管理员 author:聂箫 date：2016年1月5日
	 */
	private void appendDepartmentQuery(Finder f, Integer currDepartmentId,
			Integer inputDepartmentId, boolean superAdmin) {
		// 添加与部门查询有关的查询条件
		f.append(" and department.lft between dParent.lft and dParent.rgt");
		if (inputDepartmentId == null) {
			if (superAdmin) {
				f.append(" and dParent.id = department.id");// 必须加上这个条件
															// 否则会有重复的数据
			} else {
				f.append(" and dParent.id=:dParentId");
				f.setParam("dParentId", currDepartmentId);
			}
		} else {
			f.append(" and dParent.id=:dParentId");
			f.setParam("dParentId", inputDepartmentId);
		}
	}

	private void appendQuery(Finder f, String title, String author,
		...
		if (!StringUtils.isBlank(editor)) {
			f.append(" and bean.contentExt.editor like :editor");
			f.setParam("editor", "%" + editor + "%");
		}
		if (!StringUtils.isBlank(approver)) {
			f.append(" and bean.contentExt.approver like :approver");
			f.setParam("approver", "%" + approver + "%");
		}
		...
		if (contribute == status) {
			...
			// 待审同时包括投稿的稿件 by 聂箫 20160114
			f.append(" and (bean.status=:checking or bean.status=:contribute)");
			f.setParam("checking", ContentCheck.CHECKING);
			f.setParam("contribute", ContentCheck.CONTRIBUTE);
		} else if (rejected == status) {
			f.append(" and bean.status=:status");
						...
						if (floatValue >= 0) {
							...
							f.append(
									" and ((bean.attr[:k" + key
											...
											+ "])<:v" + key
											+ ") or bean.attr[:k" + key
											+ "]>=0)").setParam("k" + key, key)
									.setParam("v" + key, -floatValue);
						}
					} else if (operate.equals(PARAM_ATTR_GTE)) {
						if (floatValue >= 0) {
							f.append(
									" and (abs(bean.attr[:k" + key + "])>=:v"
											+ key + " and bean.attr[:k" + key
											+ "]>=0)").setParam("k" + key, key)
									.setParam("v" + key, floatValue);
						} else {
							f.append(

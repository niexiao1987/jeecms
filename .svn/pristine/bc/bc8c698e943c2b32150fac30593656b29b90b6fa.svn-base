...
public class CmsAuthenticationFilter extends FormAuthenticationFilter {
	...
	public boolean onPreHandle(ServletRequest request,
		...
		Subject subject = getSubject(request, response);
		Assertion validAssertion = (Assertion) req.getSession().getAttribute(
				SSOServlet.CONST_CAS_ASSERTION);
		if (validAssertion != null
				&& validAssertion.getPrincipal().getName() != null) {
			// 单点登录成功，validAssertion.getPrincipal().getName()为账号
			// 。。。登录处理逻
			String userName = validAssertion.getPrincipal().getName();
			if(userMng.findByUsername(userName)!=null){
				AuthenticationToken token = createToken(userName, Constants.SSO_FLAG,
						request, response);
				subject.login(token);
				isAllowed = subject.isAuthenticated();
			}
		}
		//登录跳转
